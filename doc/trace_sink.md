...
statement cache) через интерфейс `fbpp::util::TraceSink`. Клиент сам решает, подключать
...
class SpdlogSink : public fbpp::util::TraceSink {
public:
    void log(fbpp::util::TraceLevel level,
        ...
        switch (level) {
            case fbpp::util::TraceLevel::info:
                logger->info("[{}] {}", component, message);
                break;
            case fbpp::util::TraceLevel::warn:
                logger->warn("[{}] {}", component, message);
                break;
            case fbpp::util::TraceLevel::error:
...
int main() {
    ...
    fbpp::util::setTraceSink(sink.get());

    // ... работаем с fbpp ...

    fbpp::util::setTraceSink(nullptr); // отключить по завершении
}
```

`setTraceSink()` intentionally controls a process-wide diagnostic sink. If the
application uses fbpp from several threads, the sink implementation itself must
be safe for concurrent `log()` calls.
...
Сообщения формируются через `std::ostringstream` внутри `fbpp::util::trace`, поэтому
