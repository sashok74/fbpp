...
flowchart LR
    ...
    Codegen -->|query metadata introspection| DB[(Firebird database)]
...
### 2. Типизированный слой поверх runtime
...
| Query/schema generation | покрыто | `fbpp_codegen`, `query_generator`, generated descriptors |
...
- читает input/output metadata для каждого запроса
...
`fbpp_codegen` использует `AdapterConfig`:
